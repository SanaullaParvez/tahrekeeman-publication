...
(function($) {
	...
	var suppa_menu = {
		...
		eventOnClickMenuSave : function() {
			...
			// Suppamenu Part
			if( ! suppa_menu.suppaAjaxSaved ){

				// Add a Loading Animation
				$('.menu-save').before('<img id="suppaSaveLoader" style="padding:0 15px;" src="'+suppa_params.loadimg+'" >');

				// Get Suppamenu Data
				var $suppamenu_form_data = $('#update-nav-menu').serializeArray();
				var $suppamenu_data = [];

				for( var i=0; i<$suppamenu_form_data.length; i++ ){

					if(  $suppamenu_form_data[i].name.search('suppa') != -1 ){
						$suppamenu_data.push( $suppamenu_form_data[i] );
						$('#menu-to-edit').find('*[name*="suppa"]').remove();
					}
		        ...
		        jQuery.ajax({
		            ...
		            type: 'POST',
		            data: {
		            	nonce: suppa_params.nonce,
		            	action:'suppaSaveItemsData',
		            	data:$suppamenu_data,
		            },

		            success:function(data) {
		            	$('#suppaSaveLoader').remove();
		            	suppa_menu.suppaAjaxSaved = true;
