	...
	$.HandleElement.editElement = function() {
		...
		$('body').on("click", ".ig-pb-form-container .element-edit", function (e, restart_edit) {
            ...
            if ($(this).attr('data-custom-action')) {
                ...
                if ($(this).attr('data-custom-action') == "collumnAction"){

                    var activeColumn = $(this).parent().parent().parent().parent().parent().parent(".shortcode-container");
                    ...
                    var modal = new $.IGModal({
                        ...
                        buttons: [ {
                            ...
                            'click'	: function() {
                               var newValue =  modal.container.find('#param-css_suffix').attr("value");
                               var curV = $(activeColumn).find("textarea.hidden").first().text();
                               curV = curV.replace(/css_suffix="(.+?)"/, 'css_suffix="'+newValue+'"');

                               $(activeColumn).find("textarea.hidden").first().text(curV);
                    ...
                    modal.show(function(modal) {
                        var reg = /css_suffix="(.+?)"/g;

                        var curValue = (reg.exec( $(activeColumn).find("textarea.hidden").first().text()))[1];
                        var modalCont = '<div id="parent-param-css_suffix" class="control-group form-group clearfix">' +
                            '<label class="col-xs-3 control-label" for="param-css_suffix" data-toggle="tooltip" title="" data-original-title="Add custom css class for the wrapper div of this element">' +
                            '<span>CSS Class</span></label>' +
                            '<div class="controls col-xs-9">' +
                            '<input type="text" class=" form-control input-sm" value="'+curValue+'" id="param-css_suffix" name="param-css_suffix">' +
                            '</div> </div>';

                        modal.container.html(modalCont).dialog('open').dialog('moveToTop');
...
//						modal.close();
	...
	$.HandleElement.updateActiveElement = function(update_iframe, shortcode_content, item_title, before) {
        ...
        if (active_shortcode.hasClass('jsn-column-container')) {
            shortcode_content = shortcode_content.replace('[/ig_column]','');
        }
