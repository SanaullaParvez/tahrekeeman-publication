			...
			<?php $image_attributes = wp_get_attachment_image_src( get_post_thumbnail_id(), 'post-thumbnail' );
				if( $image_attributes ) {
					$img_alt = get_post_meta( get_post_thumbnail_id(), '_wp_attachment_image_alt', true );
			 		if ($add_lazy_class) {
			 			echo '<img class="lazyload" alt="'.esc_attr($img_alt).'" data-src="'.esc_url($image_attributes[0]).'" width="'.esc_attr($image_attributes[1]).'" height="'.esc_attr($image_attributes[2]).'" src="#">';
			 		} else {
			 			echo '<img alt="'.esc_attr($img_alt).'" width="'.esc_attr($image_attributes[1]).'" height="'.esc_attr($image_attributes[2]).'" src="'.esc_url($image_attributes[0]).'">';
			 		}
				}
			?>
		</div>
