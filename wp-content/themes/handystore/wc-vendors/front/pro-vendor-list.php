...
<div class="wcv-pro-vendorlist">
	...
	<div class="wcv-store-grid row">
		...
		<div class="wcv-banner-wrapper col-xs-12 col-sm-2">
		...
		<div class="wcv-description-wrapper col-xs-12 col-sm-4">
		...
		<div class="wcv-products-wrapper col-xs-12 col-sm-6">

			<div class="wcv-products-inner">
				<?php $product_images = '';
				foreach ($vendor_products_ids as $key => $id) {
						if ($key == 5 || $key == (count($vendor_products_ids)-1) ) {
							$product_images .= '<div class="product-img">'.get_the_post_thumbnail($id, 'thumbnail').'<span class="total-qty">'.sprintf( _n( '<span>%s</span> item', '<span>%s</span> items', $products_count, 'plumtree' ), $products_count ).'</span></div>';
							break;
						} else {
							$product_images .= '<div class="product-img">'.get_the_post_thumbnail($id, 'thumbnail').'</div>';
						}
				}
				echo $product_images; ?>
			</div>
