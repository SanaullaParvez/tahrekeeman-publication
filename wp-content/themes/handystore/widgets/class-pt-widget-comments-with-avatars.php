...
class PT_WP_Widget_Recent_Comments extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => __( 'Your site&#8217;s most recent comments with user avatars.' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'pt-recent-comments', __( 'PT Recent Comments' ), $widget_ops );
        ...
        if ( $comments ) {
            ...
            foreach ( (array) $comments as $comment) {
              $title = $comment->post_title;
                $output .=  '<li class="recentcomments"><div class="thumb-wrapper">'. get_avatar($comment->comment_author_email, 70). '</div>';
